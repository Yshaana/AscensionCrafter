...
def write_gate_manifest(tuning, holdout, passing, qualified, crit_met,
    ...
    manifest = {
        ...
        # 🆕 3j C5 (`AUDIT_3I_ADVERSARIAL` §9) — the STATUS, in the artifact.
        # `3f` F8c's lifecycle rule reached `primer/*.md`, `3h` A5 extended it
        # to `predictions/*.md`, and the JSONs stayed outside both — so this
        # file, the one most likely to be cited, was the only live artifact in
        # `predictions/` with no label saying whether it is current, while the
        # FROZEN `gate_manifest.json` and the new `per_ability_summary.json`
        # each had one. Written by the tool, so it cannot be forgotten.
        "status": "LIVE",
        "_status_note":
            "LIVE — the CURRENT gate manifest, regenerated by every clean-tree "
            "run of tools/audit/calibrate_crawled.py. Cite this one for "
            "today's numbers. gate_manifest.json is the FROZEN 3d record the "
            "cohort was frozen from and must not be cited as current.",
        ...
        "holdout": {
            ...
            **({} if read_holdout or not carried else {
                ...
                # 🆕 3j C6 (`AUDIT_3I_ADVERSARIAL` §9.4, owner decision
                # 2026-08-07: annotate rather than spend a re-read).
                "results_carried_forward_corpus_caveat":
                    "🛑 NOT LIKE-FOR-LIKE WITH THIS RUN'S TUNING NUMBERS, AND "
                    "THE DIRECTION IS KNOWN. These were read at a 3g commit, "
                    "BEFORE E15 was fixed — pet damage was stored twice in "
                    "ability_performance and `dps` added `pet_damage` to a "
                    "total that already contained it, so the LOGGED "
                    "denominator every one of these deltas was measured "
                    "against was INFLATED for pet-owning characters. Fixing "
                    "it moved the tuning set's slice 20.5% -> 26.3% with no "
                    "modelling change at all. So the holdout's recorded median "
                    "slice of 9.8% (n=4) is an UNDER-statement: its true "
                    "post-E15 value is somewhat BETTER, by roughly the same "
                    "mechanism and probably a similar proportion. Do NOT read "
                    "9.8% as a current like-for-like floor, and do not compute "
                    "a tuning-vs-holdout gap from these two numbers. The only "
                    "way to a comparable figure is --read-holdout, which "
                    "spends it; the owner's standing decision is to annotate "
                    "and keep it unspent until a modelling session has "
