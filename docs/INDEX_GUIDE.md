...
`ascension_index.db` (SQLite) built from `spell-export.json` (3061 spells) + `Cards.txt` (owned cards).
Rebuild anytime with `python3 build_index.py && python3 seed_confirmed.py && python3 seed_borrowed_modifiers.py` if the source exports change.
...
**spell_scaling** — one row per detected SP/AP/RAP/WEAPON coefficient (spell_id, term_type, coefficient). A spell can have multiple rows (e.g. hybrid abilities have both an SP and an AP row). **This is the damage-formula comparison table** — sort/filter it directly.
