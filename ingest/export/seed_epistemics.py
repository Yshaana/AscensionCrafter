...
QUESTIONS = [
    ...
    ('consecrated_holy_weapon_is_enchant_delivered',
     ...
     '3L UPDATE (2026-08-08, owner-gated --with-dbc run with SpellItemEnchantment.dbc in scope, 18,035 rows): the enchant route now EXISTS and half the question is CLOSED BY A NUMERIC FIELD. Enchants 23387-23395 are "Consecrated Weapon 1".."9" -> equip-spells 200819-200827 "Consecrated Holy Weapon Hidden", each carrying Ascension-CUSTOM auras [4 DUMMY, 345, 344] (stock 3.3.5 aura ids end ~316) with rank-rising BasePoints. ANCHOR, exact: rank 6 aura-345 decodes 85+1 = +86 - the 2e-measured "+86 RAW Holy SP per weapon", recovered independently, so aura 345 IS the per-weapon school-SP sheet grant, per rank, provenanced. STILL OPEN: aura 344 (R1 19 .. R6 73 .. R9 132) is the per-hit damage parameter and its formula ("based on the speed of the weapon") is underived - test it against the owner 200818 per-hit distributions (2e + MC captures) with his rank-6 enchant and weapon speeds known. Delivery modelling for crawled characters (enchant_id per snapshot_gear row names each character rank) is the 3m trigger-delivery block, owner decision 2026-08-08.',
