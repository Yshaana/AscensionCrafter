...
QUESTIONS = [
    ...
    ('consecration_and_dawn_strike_calibration_outliers',
     'The 2026-08-05 calibration against Elric\'s own log put every tested ability into one of two tight school groups - pure Holy at about 1.76x the sim base, Holystrike hybrids at about 1.40x - EXCEPT two, and those two do not look like talent-stack effects at all. CONSECRATION: sim base 56 per tick against a logged 199 non-crit average over 423 ticks, a ratio of 3.55x, far outside the Holy group. Suspect a missing damage component or a wrong rank rather than multipliers. DAWN STRIKE: sim base 476 against a logged 528, a ratio of 1.11x, far BELOW its Holystrike peers (Lightbound Cleave 1.41, Whirling Light 1.41, Dawnreaver 1.39). Its effect structure is types 121/80/31 where the others are 58/31, so the likely error is in the sim BASE being too high rather than in the multiplier being too low - effect 121 is normalized weapon damage and may be being double-counted alongside the type 31 weapon-percent effect. WHY THIS MATTERS: if either outlier is a modelling error rather than a data quirk it will distort the talent multipliers 2c fits, because a fit that includes them would drag the school constants away from the values the other five abilities agree on. Settle before fitting: re-derive both abilities\' component lists by hand from spell_effect_values, and check whether Consecration\'s logged 199 is per tick or per pulse-group.',
     'Accuracy of the Phase 2c talent multiplier fit', 'open', None, '26573,907894', '2026-08-05'),
