...
QUESTIONS = [
    ...
    # ------------------------------------------------------------- session 3n
    ('devour_mind_287865_largest_absent_key',
     'REGISTERED 3n rather than deferred a FOURTH time (SESSION_3N_PRIMER Block D; AUDIT_3M §5 item 4 - "register it or explain it"). Devour Mind 287865 is the single largest ABSENT key in the per-ability instrument at 6.63% of cohort logged damage, meaning the sim holds NO key for it at all - it is not starved, not keyed-but-zero, it is missing. It has now been carried unchanged across 3k, 3l, 3m and 3n. THE BLOCKING REASON, stated so the next session does not re-derive it: it is a TRIGGER-DELIVERED damage spell, the same family as the 58.8% absent mass generally, and the blocker is delivery modelling rather than extraction or magnitude. WHAT WOULD CLOSE IT: (1) establish whether 287865 is reached from a pressed card by EffectTriggerSpell, and if so from which - the bounded depth-2 walk in core/spells/relationships.py is the existing route and its result for this id is not recorded anywhere; (2) if it is reached, the magnitude question is then ordinary and spell_effect_values via=dbc_only may already carry it; (3) if it is NOT reached by any trigger edge, it is an out-of-catalog spell whose owner is unknown, and identifying the pressing card is the prerequisite to everything else. NOT a coefficient problem and NOT a rank problem - do not start there. ⚠ The 6.63% figure is from the per_ability_summary artifact and moves with every run; regenerate rather than quoting this rows number.',
     'The largest single absent key; delivery modelling scope',
     'open', None, '287865', '2026-08-08'),
    ('per_parse_coefficient_needs_two_stat_levels',
     'RAISED 3n Block C, and it REPLACES the no_per_parse_stats blocker rather than restating it. The Molten Core capture lifts refused:no_per_parse_stats for exactly ONE kill window: Gehennas, where the bracketing stat blocks agree (AP 488 held, brackets -607s/+822s) and logged 319.8s is separated from wall 379.8s. Lucifron and Magmadar refuse for want of a block BEFORE the window; Garr refuses because its bracketing blocks DISAGREE across five stats at once (AP 488->471, Str 214->201, Agi 112->99, Int 359->346, crit 32.36->31.96); Baron Geddon has no kill. THE NEW BLOCKER IS ARITHMETIC, NOT DATA: one admissible window is ONE (stat, damage) point per ability, and a coefficient is a SLOPE - damage = flat + coeff x stat is under-determined by a single point, and every coefficient fits it with the flat absorbing the difference. Fitting one would produce a confident number determined entirely by an assumption about the flat, which is what infer_coefficient exists to refuse. WHAT LIFTS IT: a second ADMISSIBLE window at a materially different stat level for the SAME ability - not merely a second parse. This is a CAPTURE-PROTOCOL problem, not a data-availability one: the stats already move across this raid (AP 488 -> 471 -> 331), and the protocol fix is one line - take a stat block immediately BEFORE and immediately AFTER each pull, so every window is bracketed tightly enough to be admissible. Tool: tools/analysis/pair_parses_to_stats.py.',
     'Phase 3 criteria 3-full and 4; every per-parse coefficient',
     'open', None, None, '2026-08-08'),
]
