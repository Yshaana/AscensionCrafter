...
QUESTIONS = [
    ...
    ('trigger_attributed_coefficients_not_in_spell_scaling',
     'Hammer from the Heavens\' confirmed +9.1% SP / +9.1% AP (three-source, session 1x) existed only in doc prose — spell_scaling had no rows for 282987, so the resolver served the flat 122-145 without the stat terms (~45% understatement at AP 584/SP 533). DECISION (owner, 2026-08-05, same day): coefficients live on the trigger TARGET — seed_hand_coefficients.py now owns tier-3 db_ascension_gg rows keyed to 282987, never duplicated onto the cards. REMAINING (why still in_progress): 2b must teach the ability model to pull spell_scaling rows per component source_spell_id (bounded, single-path, confidence=inferred when trigger-reached, mirroring 1b\'s walk rules) — until then the rows are queryable but not yet served through a card\'s profile.',
     'Sim accuracy for trigger-reached abilities (HftH first among them)', 'in_progress', None, '282983,282984,282987', '2026-08-05'),
