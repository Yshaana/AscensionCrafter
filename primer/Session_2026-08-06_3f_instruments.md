...
| **F6** | The freeze had **two exits and only one was reported** — scoring-loop losses never reached the manifest | `still_qualifying` / `scored` / `excluded_after_selection`, and **two assertions REFUSE to write a manifest whose members do not add up** — ⚠ **corrected `3g` G8: they are regression guards on the CALLER'S WIRING, not a runtime data invariant.** Given `_completeness_sql`'s `GROUP BY ep.character_id`, both reduce to arithmetic identities and no data condition can trip them. They stay, because reverting the caller does turn them red; the CLAIM was the overstatement |
