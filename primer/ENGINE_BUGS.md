...
| M52 🆕 | delete the `"crit_damage"` entry from `per_event` in `ability_model.expected_cast` | 1 | ✅ the entry, plus `crit_damage_each` in `expected_hit`'s breakdown. **This is the exact pre-fix state**: `tiers.py:861` read `ev.get("crit_damage")` and *nothing in the tree wrote that key*, so Righteous Vengeance was modelled as zero for every character since `2e` — 0 sim rows for 61840 cohort-wide against 9 characters logging it (`3k` B3) | — |
| M53 🆕 | stub the ownership gate: `holds_rv = True` in `_add_swing_sources` | 1 | ✅ the derived `any(... in RIGHTEOUS_VENGEANCE_CARD_SPELL_IDS ...)`. Populating M52's key **without** this gate gives all 35 cohort characters an RV row when only 9 log one — phantom production, which the `3k` work order names as being as wrong as a missing key (`3k` B3) | — |
| M54 🆕 | in `parse_enchant_record` (`ingest/dbc/build_dbc_index.py`), read `amounts_min` from the UNSIGNED slots — `slots[5:8]` instead of `slots_i[5:8]` | 2 | ✅ the `slots_i` (signed) read. The fixture's EffectPointsMin is `-5`, which the mutation decodes as `4294967291`; the second red is the non-stock-field_count arm asserting the same prefix. Owner-gated-path insurance per `3b`: only `--with-dbc` runs this code, so the parse core is pure (bytes in, dict out) and harness-tested on every clone (`3l` D2) | — |
| M55 🆕 | in `cli/gear_tiers.py run()`, delete BOTH `REFUSED` arms — the `snapshots_kept == 0` empty-window arm and the `insufficient_sample` arm (both sit between `scoping = ...` and the `as_json` branch; restore them there) | 2 | ✅ the two arms. The pre-C1 failure mode was that no production caller existed at all, so "whatever comes back is the answer" — mutated, the empty Molten Core window falls through to the report path (and in fact crashes on the thin return shape, which the harness converts to a clean FAIL rather than a masked traceback). The live case is real: MC opened 2026-08-07T18:00Z with zero corpus snapshots on its side (`3l` C1) | — |
| M56 🆕 | in `per_key_table` (`tools/audit/per_ability_accuracy.py`), divide each group's share by the GROUP's own summed logged damage instead of `total_logged_all` | 1 | ✅ the `total_logged_all` denominator. The committed per-key table is what every target-pick and audit reads (`AUDIT_3K` §3.2: the `3k` target list had no committed source); the arm sums its per-state shares against independently-computed aggregates, E1-style — two code paths over one row set (`3l` B0) | — |
| M57 🆕 | **two red halves, both run.** (a) change one digit in the `<!-- GENERATED derived-figures -->` block in `predictions/CALIBRATION_TOLERANCE.md` (`3.3×` → `3.9×`); (b) delete the block's BEGIN marker, so it is not *found* rather than not *equal* | 1 each | ✅ **run `py tools/audit/render_band_table.py` and paste the second block** — regeneration, which IS the fix. 🛑 `git checkout` of the doc also goes green and is **not** the registered path: a revert proves only that the file used to agree. The green run above restored the marker and re-pasted from the tool. Guards the sentences `[3j-C3]` left unowned — `3l` regenerated the band table correctly and left "At **26.3%**", "a **3.8×** rise" and "roughly **ONE QUARTER**" stale within twenty lines of it, the **third** staling of this file inside its own assertion's blind spot (`3m` pre-flight, `AUDIT_3L` F11) | — |

| M58 🆕 | in `canary_check` (`tools/scrapers/crawl_ascensionlogs.py`), replace the carry-over arm with the pre-fix run-normalised rate (`now/runs_now < before/runs_prev * CANARY_DROP_RATIO`) | 4 | ✅ the carry-over arm. Reds the real 2026-08-07→08 transition at `2.0 rec/run vs 12.0` — the state the daily job was stuck in at every logon. **Arm 4 goes red too, which is the informative part: the old canary could not catch the payload-shape break either**, so the fix is not a loosening (`3m` pre-flight, `AUDIT_3L` F17) | — |
| M59 🆕 | in `canary_check`, delete the `if not carry:` branch so an empty carry-over set falls through to the ratio | crash | ✅ the branch. Reds as a `ZeroDivisionError` — the branch is the only thing between an empty carry-over set and `0/0`, and its job is to SAY the guard could not run rather than pass silently (`3b`'s "OK - game is closed" printed without checking). A coarse red, recorded as such | — |

| M60 🆕 | in `core/sim/tiers.py :: _add_swing_sources`, delete the `elif crit_damage > 0 and holds_rv is None:` warning branch **but leave the phrase "WITHOUT a card-ownership check" behind in a comment** | 1 | ✅ the branch. 🚨 **The isolating mutation for `3m` A3.** The pre-A3 arm was `"WITHOUT a card-ownership check" in inspect.getsource(...)`, and under this mutation that expression evaluates **True** — measured, printed as `OLD source-text arm would report: PASS (vacuous)` — while the repaired arm (which CALLS the function with `build_spec=None` and a crit-damage row and asserts on the returned `warnings`) goes red. A mutation that deletes the phrase as well reds BOTH arms and proves nothing; leaving the phrase is what isolates the defect (`3m` A3, `AUDIT_3L` F9) | — |
| M61 🆕 | remove `20496` from `core/sim/talents.py :: BONUS_TERM_ONLY_TALENT_IDS` (or empty the set) | 2 | ✅ restoring the id. The weapon half takes Improved Cleave's ×2.20 again and the blended effective multiplier returns to 2.20 from 1.40, i.e. 330 where the intended model gives 210. **This is the exact pre-2026-08-10 server behaviour**, so the mutation is not hypothetical — it is what the sim did until this commit (`3m` B) | — |
| M62 🆕 | in `damage_multiplier`, drop the `component == "weapon"` skip | 2 | ✅ the skip. Same two arms red by the other route — the id set is consulted and does nothing, which is the failure mode where a registered scope exists but is never applied. ⚠ Arm 3 asserts the OPPOSITE direction (an ordinary op-8 talent STILL reaches the weapon term), so a mutation that makes every modifier bonus-only cannot pass the set either (`3m` B) | — |

| M63 🆕 | `tools/audit/calibrate_crawled.py:207 :: WEAPON_SLOTS = {16: "main_hand", 17: "off_hand"}` (the pre-`3l` value). ⚠ **Site corrected `3n` pre-flight (`AUDIT_3M` F4)** — this row said `core/sim/tiers.py`, which has no such constant, so the registered edit applied there is a **no-op and the harness stays green**: a false revert, exactly what this table exists to prevent. Applied to the real site it is red as claimed. | 1 | ✅ `{15: main_hand, 16: off_hand}`. The corpus stores weapons at slot 15, so the old map simmed **13 of 41 cohort members weaponless** and every double-2H character swung the wrong single weapon. `3l` fixed it and registered nothing (`3m` C5, `AUDIT_3L` F2) | — |
| M64 🆕 | delete `base += ap / 14.0 * float(speed)` in `expected_swing` | 1 | ✅ the term. Asserted as ARITHMETIC, not by name: a flat-100 weapon at speed 3.0 on 1,400 AP must swing for **400**, not 100. `3l` measured Ryno at 122 sim per swing against 1,382 logged and this is what closed it (`3m` C5) | — |
| M65 🆕 | restore `if mh <= 0: return` at the top of `_add_swing_sources` | 1 | ✅ its removal. A weaponless build still derives Righteous Vengeance — RV needs no weapon, and the early return skipped it along with the autos. Deyindra and Shana both held the card, pooled positive crit damage, and derived nothing (`3m` C5) | — |
| M66 🆕 | force the per-snapshot slot-convention detector to one branch (drop the `15 in weapon_slot_ids` test in `build_spec_for`) | 1 | ✅ the detector. The corpus MIXES two conventions — 27 server-numbered vs 14 API-style — so either fixed map strands one half; `3l` found this only because fixing the first half falsified its own prediction (`3m` C5) | — |

| M67 🆕 | in `core/logs/encounters.py :: kill_window`, window on the boss NAME — `start = encounters[0]["first"]` instead of the kill GUID's own `first` | 2 | ✅ the kill GUID's own start. The wipe folds back in and the window inflates 100s → 400s on the fixture; on the real Molten Core capture **Gehennas reads 603.6s instead of 379.8s and Garr 2,138.6s instead of 382.6s** — a 5.6× inflation on the capture whose entire value is per-parse calibration (`3m` D, `AUDIT_3L` F13) | — |
| M68 🆕 | delete the no-kill refusal and fall back to the LONGEST attempt, ending at its last event | 1 | ✅ the refusal. This is the mutation the refusal's own wording names — *"returning the longest or the last attempt would be a guess wearing a measurement's clothes"*. ⚠ A first draft (`kills = [encounters[-1]]`) crashed on `died = None` instead: a TypeError is red but it is a CRASH, not the behaviour under test. Sharpened to a plausible implementation, which is the only kind worth mutating to (`3m` D) | — |
| M69 🆕 | delete the multi-kill refusal so `kills[0]` is picked silently | 1 | ✅ the refusal. A boss killed twice in one log has no unambiguous "the kill", and resolving it by list order is the failure rank resolution already refuses (`3m` D) | — |
| M70 🆕 | `core/sim/apl_gen.py :: off_gcd = gt in ("none", "off")` **without** the `or next_swing` clause, plus delete `fast_sim`'s swing-budget branch so next-swing abilities fall back to `gcd_budget / occupancy` | 1 | ✅ the swing budget (`3n` B leg 1): next-swing abilities leave the GCD-filler ranking and are bounded by `window / main-hand swing interval`. 🔬 **This mutation is the code that ACTUALLY SHIPPED from `3e` through `3m`** — not invented. Under it, 10 of 35 tuning-set members have a next-swing ability allocated zero casts, Blix's Lightbound Cleave among them at 41% of his logged damage | `[3n-B]` |
| M71 🆕 | keep leg 1 but pass `next_swing_casts=0.0` to `_add_swing_sources` — the plausible half-fix, from someone who fixed the clock and did not notice the word *replacement* in `schema.py:341` | 1 | ✅ leg 2: main-hand autos reduced by the next-swing casts, clamped to the swings available, main hand only. Without it the sim counts one swing **twice** — once as an auto and once as the ability that replaced it | `[3n-B]` |
| M73 🆕 | `core/sim/tiers.py` — delete the `swing_budget = max(0.0, swing_budget - casts)` decrement, so every on-next-swing ability sees the FULL budget (two Cleaves riding one white swing) | 1 | ✅ the swing budget is SHARED. 🚨 **This is one of the two mutants `AUDIT_3N` F5 ran and found GREEN across the entire harness**, because every fixture then held exactly one next-swing ability. On the new `multi_next_swing` fixture it reds with the precise signature predicted: joint casts **38.951 = 2 × 19.475**, both abilities allocated the whole budget. ⚠ `cp_melee` STILL PASSES under this mutant — the old fixtures' blindness, demonstrated rather than asserted | `[3o-D]`, `[3n-B]` |
| M74 🆕 | `core/sim/tiers.py :: _add_swing_sources` — apply the next-swing replacement to the OFF hand as well (`oh = max(0.0, oh - replaced)`), which the code's own comment calls "a different, invented mechanic" | 1 | ✅ replacement is MAIN HAND ONLY. The other mutant F5 found green: **no arm read `auto_oh` at all** before `3o` Block D. Under it the off-hand auto row disappears entirely (0.0 against the 19.5 its own 3.609 s interval gives) | `[3o-D]` |
| M75 🆕 | `addons/…/AscensionCrafterExport.lua :: takeSnapshot` — drop the `full and … or …` branch so every snapshot emits `gatherText` (the full export). The plausible simplification: *"there is already one emitter, why keep a second path?"* | 1 | ✅ the spec's **size discipline**: per-combat snapshots are stat blocks only, the gear/board dump once per session. Under the mutant the blob grows **11,542 → 17,500 chars for two pulls**, and a 5-boss dungeon stops fitting in one paste | `[3o-B]` (`check_addon_lua.py`) |
| M72 🆕 | `core/sim/buffs.py :: CONSECRATED_WEAPON_IMBUE.attack_power` — restore `88.0` | 1 | ✅ the imbue's AP grant equals the capture's measured per-step delta. 🔬 **This mutation is the code that ACTUALLY SHIPPED from `3k` through `3n`** — not invented: `3n` retired the +88 in both seeds and left it executing here, for 6 of the 35 tuning-set members. The arm reads the three-step export's own bytes (`[80.0, 80.0]`) rather than a literal copied beside the constant | `[3o-C]` |
| M77 🆕 | `core/logs/encounters.py :: derive_capture_gaps` — truncate the derived gap to whole seconds (`float(int(...))`), which is exactly the shape of the hand-typed constant it replaced and a plausible "tidy up the float" edit | 1 | ✅ the gap is derived at the precision the client writes. 🔬 **The mutant reproduces the defect EXACTLY**: gap 60.000 s and Gehennas logged **319.8 s** — the figure `3n` published — against the derived 59.764 s / **320.084 s**. `logged_seconds` is the denominator of every per-parse coefficient, so this bias is one-directional | `[3o-B2]` |
| M76 🆕 | `core/builds/stat_block.py` — raise `StatBlockError` on any `Key: value` whose key is not in a known set. The plausible hardening: *"be strict about input rather than silently ignoring part of the block"*, and it reads like an improvement | 1 | ✅ the parser SKIPS unknown lines, which is what makes the addon's new `SnapshotReason:`/`Target:`/`Weapon*:` fields additive. Under the mutant every `/ace` session snapshot is refused — the capture format and the parser would have diverged silently until a live dungeon produced an unparseable blob | `[3o-B1]` |
...
| | |
|---|---|
| **Check** | ✅ **FOUR registered failing checks** — `[cp_melee] / [dot_caster] / [frost_mage] / [multi_next_swing] fast_sim allocates GCDs to more than one filler`, all in `EXPECTED_FAILURES`. 🆕 `3o` Block D added the fourth with its fixture: on that board **0 of 2** on-GCD fillers cast, because two next-swing abilities subscribe the whole swing clock and the surviving GCD filler takes the rest — the same shape reaching its extreme (`grep -n "more than one filler" tools/audit/check_sim_engine.py` — a line range stood here and drifted, `3i` A5). Also surfaced per ability as a `warnings` entry from `core/sim/tiers.py :: _mixed_damage_warning`. ⚠ The row read *"none yet"* until `3f` F8, contradicting this file's own every-entry-is-a-failing-check invariant (the header block above the registry; it read "line-15" here and that line number drifted too) |
