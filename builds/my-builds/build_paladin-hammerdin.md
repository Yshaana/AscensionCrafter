...
**Purpose:** Continue work on my Ascension (WoW private server) Paladin character. **Classless server, Season 10, Wildcard mode.** Pair this with `primer/Ascension_Context_Primer.md` (v19) and `data/derived/ascension.db` (queryable spell/card index — see primer §2a).
...
**⚠ Addendum (2026-08-04, from `builds/wip/wip_winds-of-winter-frostblade.md` §4):** the underlying "is Duality's SP amp even real" question — not this specific character's board state — has since been answered on the *same character* (Elric) via a clean controlled test: identical gear, **empty talent/ability spec** (no Lunar Guidance to confound the result), Path of Strength vs. Path of Duality compared back-to-back using a custom export addon (`addons/AscensionCrafterExport/`). Result: Spell Power (all schools) 229 → 434, a clean **1.895x multiplier** — the amp is real. Int→melee-crit and Agi→spell-crit conversions also confirmed real (melee crit rating +59, spell crit rating +8 despite Agility being unchanged between the two exports — proving that channel doesn't exist at all under Path of Strength). **The v4 retraction above was itself the contaminated measurement** — a single item swap on this same talent-loaded board, exactly the kind of small-magnitude test the primer's own methodology warns against (§5, `method_sample_size`). Full data in `seed_confirmed.py`'s `duality_sp_amp_confirmed_reverses_retraction` fact. **New complication found in the same test, not previously known:** Attack Power dropped to 0.548x the Path of Strength value under Duality, contradicting the tooltip's `AP = highest of Str or Agi` claim (Strength was still well above Agility) — flagged as `duality_attack_power_anomaly`, unresolved whether bug or undocumented reduced conversion rate. **This URGENT flag can be downgraded** — Duality is confirmed mechanically functional; what's left open is Elric's specific *current* board/path selection (unrelated to whether the mechanic works) and the new AP anomaly.
...
2. ~~Hammer from the Heavens' coefficients (hidden sub-spell 282987) — 22.1% of damage~~
   ...
   **Free verification available:** 224–247 per non-crit hit is checkable against any combat log
   ...
   never by a tooltip reference, which is why it took a trigger-chain walk to find.
